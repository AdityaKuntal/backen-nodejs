...
exports.updateBlogs = async function (req, res) {
  try {
    ...
    let {title, body, tags, subcategory,category,isdeleted} = {...dataForUpdate}
    ...
    let updateBlog = await blogModel.findOneAndUpdate(
      { _id: blogId, isPublished: true, isdeleted: false },
      {
...
exports.loginUser = async function (req, res) {
  try {
    ...
    let getUser = await authourModel.findOne({ email: email }).select({ password: 1 })
